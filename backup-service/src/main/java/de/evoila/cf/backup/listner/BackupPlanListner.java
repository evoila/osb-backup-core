...
@EnableKafka
public class BackupPlanListner {
        private static final Logger log = LoggerFactory.getLogger(BackupPlanListner.class);
        private KafkaProperties kafkaProperties;
        private BackupPlatformRepository backupPlatformRepository;
        private ServiceDefinitionRepository serviceDefinitionRepository;

        public BackupPlanListner(KafkaProperties kafkaProperties) {
                this.kafkaProperties = kafkaProperties;
        }

        @Bean
        public DefaultKafkaConsumerFactory<String, BackupPlanEvent> backupPlanConsumerFactory() {
        ...
        @Bean
        public ConcurrentKafkaListenerContainerFactory<String, BackupPlanEvent> backupPlanKafkaListenerContainerFactory() {

                ConcurrentKafkaListenerContainerFactory<String, BackupPlanEvent> factory =
                        new ConcurrentKafkaListenerContainerFactory<>();
                factory.setConsumerFactory(backupPlanConsumerFactory());
                factory.getContainerProperties().setAckMode(ContainerProperties.AckMode.MANUAL_IMMEDIATE);
                return factory;
        }

        @KafkaListener(
                topicPattern = "Backup-Plan-#(#{T(org.thymeleaf.util.StringUtils).join(catalogServiceImpl.getServiceIdsWithoutHyphen(),\"|\")})-.*",
